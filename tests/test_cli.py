...
def mock_config():
    return {
        "run": {"lookup-folder": "../navamai-vault/Code", "save-folder": "../navamai-vault/../../Apps"},
        "provider-model-mapping": {"provider1": ["model1", "model2"]},
        "test": {"ask": "test prompt", "vision": "vision prompt", "image-path": "../navamai-vault/Images/hackathon.jpg"},
        "ask": {"provider": "mock_provider", "prompts-folder": "../navamai-vault/Prompts"},
        "refer-section": {"provider": "mock_provider", "lookup-folder": "../navamai-vault/Raw"},
        "intents": {"provider": "mock_provider", "lookup-folder": "../navamai-vault/Intents", "save-folder": "../navamai-vault/Embeds"},
        "merge": {"lookup-folder": "../navamai-vault/Posts", "dest-suffix": "expanded", "merge-suffix": "merged", "placeholder": "[merge here]", "prompt-prefix": "> Prompt:"},
        "validate": {"provider": "mock_provider", "lookup-folder": "../navamai-vault/Intents", "save-folder": "../navamai-vault/Embeds", "validate-prompt": "Validate this"},
        "vision": {"provider": "mock_provider", "model": "vision_model", "lookup-folder": "../navamai-vault/Gather/images", "system": "System prompt", "save": True, "save-folder": "../navamai-vault/Vision"},
...
def test_run(mock_process_markdown, mock_file_select, mock_get_provider, mock_load_config, runner, mock_config, mock_provider_instance):
    ...
    mock_process_markdown.assert_called_once_with("../navamai-vault/Code/selected_file.md", app_folder="../navamai-vault/../../Apps")

@patch("navamai.cli.configure.load_config")
@patch("navamai.cli.configure.save_config")
@patch("navamai.cli.utils.get_provider_instance")
@patch("navamai.cli.metrics.save_test_summary")
def test_test(mock_save_summary, mock_get_provider, mock_save_config, mock_load_config, runner, mock_config, mock_provider_instance):
    mock_load_config.return_value = mock_config
    mock_get_provider.return_value = mock_provider_instance
...
def test_validate(mock_diff, mock_parse, mock_get_provider, mock_load_config, runner, mock_config, mock_provider_instance):
    mock_load_config.return_value = mock_config
    mock_get_provider.return_value = mock_provider_instance
    mock_parse.return_value = [("Intent1", "Prompt1")]
    mock_diff.return_value = 10.5
    
    with patch("builtins.open", mock_open(read_data=b"Mock content")), \
...
def test_intents_existing_response(mock_update, mock_confirm, mock_intent_select, mock_parse, mock_file_select, mock_get_provider, mock_load_config, runner, mock_config, mock_provider_instance):
    mock_load_config.return_value = mock_config
    mock_get_provider.return_value = mock_provider_instance
    ...
    with patch("builtins.open", mock_open(read_data=b"Mock content")), \
        ...
        assert "Operation cancelled. Existing file will not be replaced." in result.output

@patch("navamai.cli.configure.load_config")
@patch("navamai.cli.utils.get_provider_instance")
@patch("navamai.cli.markdown.parse_markdown_sections")
@patch("navamai.cli.markdown.diff")
def test_validate_response_file_not_found(mock_diff, mock_parse, mock_get_provider, mock_load_config, runner, mock_config, mock_provider_instance):
    mock_load_config.return_value = mock_config
    mock_get_provider.return_value = mock_provider_instance
    mock_parse.return_value = [("Intent1", "Prompt1")]
...
def test_vision_with_url(mock_has_vision, mock_requests_get, mock_display, mock_resize, mock_file_select, mock_get_provider, mock_load_config, runner, mock_config, mock_provider_instance):
    ...
    with patch("navamai.model_vision.tempfile.NamedTemporaryFile") as mock_temp_file, \
        ...
        result = runner.invoke(vision, ["-u", "https://example.com/image.jpg", "Test prompt"])
        assert result.exit_code == 0
...
@patch("navamai.cli.utils.get_provider_instance")
@patch("navamai.cli.markdown.file_select_paginate")
def test_refer_no_file_selected(mock_file_select, mock_get_provider, mock_load_config, runner, mock_config, mock_provider_instance):
    mock_load_config.return_value = mock_config
    mock_get_provider.return_value = mock_provider_instance
...
@patch("navamai.cli.markdown.parse_markdown_sections")
@patch("navamai.cli.markdown.intent_select_paginate")
def test_intents_no_intent_selected(mock_intent_select, mock_parse, mock_file_select, mock_get_provider, mock_load_config, runner, mock_config, mock_provider_instance):
    mock_load_config.return_value = mock_config
    mock_get_provider.return_value = mock_provider_instance
    mock_file_select.return_value = "../navamai-vault/Intents/intents.md"
    mock_parse.return_value = [("Intent1", "Prompt1")]
...
@patch("navamai.cli.configure.load_config")
@patch("navamai.cli.utils.get_provider_instance")
def test_id_default_section(mock_get_provider, mock_load_config, runner, mock_config, mock_provider_instance):
    mock_load_config.return_value = mock_config
    mock_get_provider.return_value = mock_provider_instance

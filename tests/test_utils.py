...
from navamai.utils import trail, get_provider_instance
...
def test_trail_decorator():
    ...
    with patch('navamai.utils.datetime', mock_datetime), \
         patch('navamai.utils.open', mock_open()) as mock_file, \
        ...
        mock_yaml_dump.assert_called_once_with(expected_log_entry, mock_file(), default_flow_style=False)
...
def test_trail_decorator_with_string_result():
    ...
    with patch('navamai.utils.open', mock_open()) as mock_file, \
         patch('yaml.dump') as mock_yaml_dump, \
        ...
        assert log_entry['command'] == 'sample_function_string '  # Note the trailing space

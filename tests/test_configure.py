...
def test_load_config_with_file():
    with patch("os.path.exists", return_value=True), patch("builtins.open", mock_open(read_data=mock_config_yaml)):
        from navamai.configure import load_config
...
def test_edit_config_existing_key():
    ...
    with patch("navamai.configure.load_config", return_value=mock_config_data), \
         patch("navamai.configure.save_config") as mock_save_config:
...
def test_edit_config_type_conversion():
    ...
    updated_config = {"test": {"number": "43"}}  # Expecting the value to remain as a string

    with patch("navamai.configure.load_config", return_value=initial_config), \
         patch("navamai.configure.save_config") as mock_save_config:
        from navamai.configure import edit_config
...
def test_resolve_model():
    with patch("navamai.configure.get_model_mapping", return_value=mock_config_data["model-mapping"]):
